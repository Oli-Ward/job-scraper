...
logger = logging.getLogger("JobScraper.UnicornFactory")

BASE_URL = "https://www.unicornfactory.nz"

def fetch_unicornfactory_jobs():
    """
    Scrapes Unicorn Factory job listings via JavaScript rendering.
    Note: Unicorn Factory appears to not have public job listings on their website.
    """
    ...
    text = soup.get_text()
    if not text or len(text) < 500 or 'job' not in text.lower():
        logger.debug("No job listings found - Unicorn Factory may not have public job board")
        return pd.DataFrame()
    ...
    if not job_cards:
        logger.debug("No job cards found - Unicorn Factory may not have public job board")
        return pd.DataFrame()
